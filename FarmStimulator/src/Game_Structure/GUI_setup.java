...
public class GUI_setup {
	...
	public void btnCancel() {											
		if ((game_state == 1) && (condition == 3)) { 					//Act as clear button in this stage
		...
		else if ((game_state == 1) && (condition == 15)) { 				//Cancel and go back previous stage
			Text_field.setText("");									//Clear textfield to avoid confusion
		...
		else if((game_state == 1) && (condition == 8)) {
			
		    	  Dialog.setText("<html>Hmmm....You look young, how old are you?<br>"
		    			  + "Type in digit only...");
		    	  condition = 16;
		    	  Text_field.setText("");
			...
			Dialog.setText("<html> Nicely done " + Game_Profile.getFarmer_name() + " [" +  Game_Profile.getAge() + " years old] .<br>"			// Display system message to user
	...
	public void btnProceed() {				
		if ((game_state == 0) && (condition == 0)) { 					//Introduction on key
			...
			Dialog.setText( temp_str + " Farm. Is this name ok?");						// Display system message to user
		 	condition = 15; 															//update vrb condition to avoid meeting previous if statement condition
		} //[1][7]
		
		else if ((game_state == 1) && (condition == 15)) {																//get farmer age
	    	  Dialog.setText("<html>You look young, how old are you?<br>"												//Forget to implement at first, add in last
	    			  + "Type in digit only...");
	    	  Text_field.setText("");																					//clear text field
	    	  condition = 16;
			
		}
		else if ((game_state == 1) && (condition == 16)) {	
			temp_str = Text_field.getText();
	    	  Dialog.setText("<html>Is this Correct?");
			String pattern = "\\d+";
		    Pattern r = Pattern.compile(pattern);
		    Matcher m = r.matcher(temp_str);																		// allowed input num
		      if (m.find()){																						//Only allows digit input 
	
		    	  Game_Profile.setAge(Integer.parseInt(temp_str));
		    	  condition = 8;
		      }else {
		    	  Dialog.setText("<html>Hmmm....You look young, how old are you?<br>"
		    			  + "Type in digit only...");
			...
			Dialog.setText("<html> Nicely done " + Game_Profile.getFarmer_name() + " [" +  Game_Profile.getAge() + " years old] .<br>"
					+ "Now lets get back to your grandpa Farm and began your new episode.<br>"
